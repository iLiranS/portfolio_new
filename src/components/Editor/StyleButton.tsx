...
const StyleButton = ({ active, style, label, onToggle,title,className }: Props) => {
  ...
  return (
    <button  onClick={_onToggleClick} title={title}
      className={`${active ? 'opacity-100 bg-darkBG/30 dark:bg-lightBG/30' : 'opacity-50  bg-darkBG/10 dark:bg-lightBG/10'} ${!active && `hover:opacity-70`} select-text rounded-md aspect-square h-10 grid place-items-center p-2 `}
    >
